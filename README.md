...
[![Version](https://img.shields.io/badge/version-0.9.2-orange)](https://github.com/MengBad/astrbot_plugin_anima/releases)
...
| **独立端口仪表盘 (v0.9.2)** | `dashboard_standalone_enabled` | 在 AstrBot WebUI 之外另起一个独立 HTTP 端口提供同一个仪表盘网页，像别的插件那样用 `http://ip:端口` 直接打开。默认关闭、默认仅绑本机、强制 token 鉴权。用 `/anima_dashboard_url` 取带 token 的地址 |
...
| danger_core_mutation | bool | `false` | 🔴 自主修改核心人格（需同时开启 `_confirm`） |
| danger_core_mutation_confirm | bool | `false` | 🔴 二次确认 |
| danger_identity_crisis | bool | `false` | 身份危机 |
| danger_memory_infection | bool | `false` | 🔴 记忆感染（需同时开启 `_confirm`） |
