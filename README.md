...
在 **v1.1.0** 中，Anima 已将 **SylannEngine** 作为核心运算状态机进行了直接深度整合。

*   **关系与自我叙事的结合**：两者合在一起才是一个完整的"人"：有关系物理学，也有自我叙事。Sylanne 负责"关系走到了哪里"，Anima 负责"角色知道自己是谁"。
*   **引擎底层动态路由**：当 Anima 在 live 环境运行时，其伤痕敏锐度、双 EMA 人格漂移、主动欲望决策、sheaf 关系图谱与数据落盘等基础 mixins 将**直接动态路由至 SylannEngine**，确保运算在数学上的严密与高效。
*   **自动化测试降级回退**：在自动化测试阶段，系统会自动激活 Mock 回退机制，使得插件可独立执行并完美通过全量单元测试。
...
### 可观测与健康诊断（v0.9.x + v1.1.0）

| 机制 | 开关 | 说明 |
|------|------|------|
| 运行仪表盘（文本） | `dashboard_enabled` | `/anima_stats` 查看今日各子系统运行统计（内部 LLM 调用次数 / 沉淀 / 主动发言拦截 / 存储），判断 token 烧在哪 |
| 运行仪表盘（网页） | `dashboard_enabled` | WebUI 左侧统一 Anima Plugin Page 入口内展示，图形化呈现上述统计并自动刷新 |
| **Cognitive Observatory (v1.2.5)** | `sylanne_webui_enabled` | 统一 Portal 内提供 Runtime Timeline、Prompt Debugger、State Inspector、Memory Explorer、Memory Recall Replay、Desire Dashboard、Desire Evolution、Scar Explorer、Personality Drift、Reasoning Trace 与 Session Replay；所有视图默认脱敏，只展示统计、形状、分桶与指纹 |
...
| **子系统运行追踪 (v1.1.0)** | `dashboard_enabled` | **1.1.0 新增**：实时监控大类模块（Core, Models, Memory, Autonomy, Safety）运行状态，并在网页主页以呼吸灯卡片呈现。 |
| **增量过滤日志流 (v1.1.0)** | `dashboard_enabled` | **1.1.0 新增**：极低开销的 $O(1)$ 级 Logger 前缀过滤隔离日志；WebUI 支持大类下拉筛选，只拉取增量差异（`since_ts`），规避爆前端内存与频繁刷新开销。 |
...
| 指令 | 说明 |
|------|------|
| `/anima_help` | 指令总览（按日常 / 运维 / 能力 / 高级分组，含前置条件） |
...
| `/anima_dashboard_url` | 获取统一 WebUI 的访问地址（含 token，需开启 `sylanne_webui_enabled`） |
| `/anima_capabilities_audit` | 体检个人能力库健康状况（总数 / 0 使用数 / 疑似自封高分数，只读） |
| `/anima_scan_rejects` | 扫描知识库拒答/注入污染规模（只读，不删除） |
...
| dashboard_history_days | int | `30` | **v1.0.0**：历史趋势保留天数，跨天自动归档当天统计供趋势图展示 |
...
| `capability_promote_enabled` | `false` | **v0.9.10**：晋升 Top-K 能力为命名独立工具，打破"没用过→不被发现→没用过"死锁。🔴 增 token（命名工具变长输入）。**强烈推荐开启**以闭合能力使用闭环；关闭时行为与 v0.9.4 一致 |
| `capability_promote_top_k` | `3` | **v0.9.10**：晋升为命名工具的能力数上限 |
| `capability_match_hint_enabled` | `true` | **v0.9.10**：命中时注入定向提示，引导模型优先调用匹配能力（本地词法匹配，近乎免费） |
| `capability_match_hint_threshold` | `0.2` | **v0.9.10**：注入定向提示的最低词法相关性阈值 |
| `capability_match_hint_backend` | `"lexical"` | **v0.9.10**：相关性计算后端，lexical（默认本地）/ embedding（可选，不可用自动降级） |

**风险提示**：v0.6+ 新增的自主能力系统让角色拥有更强的自我演化能力。请务必理解每个配置的风险后再开启。

> **v0.9.10 能力使用闭环**：v0.9.4 修了"只增不减"，v0.9.10 进一步闭合"能力极少被调用"——通过晋升 Top-K 能力为命名工具（`capability_promote_enabled`，强烈推荐开启）+ 命中时的定向提示（`capability_match_hint_enabled`，默认开、近乎免费）让能力真正被发现和使用，并用 `/anima_capabilities_audit` 观察 `total_usage` 是否上升。
