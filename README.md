...
[![Version](https://img.shields.io/badge/version-1.2.5-orange)](https://github.com/MengBad/astrbot_plugin_anima/releases)
...
v1.2.5 将 WebUI 收束为一个统一 AstrBot Plugin Page：`Anima Portal`。旧 dashboard 与 capability-tree 仍保留为内部 iframe 页面，不再作为多个顶层入口暴露。
...
| StateStore Audit | 只读状态源拓扑审计，展示 file/runtime/KV 来源、元数据指纹与 snapshot/diff/rollback 能力缺口 |
...
共享 AstrBot Plugin Page 路径使用 `/astrbot_plugin_anima/api/...`。独立 Sylanne WebUI 仍使用 `/api/...`。Portal 会自动识别当前入口并选择正确路径。
